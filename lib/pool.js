...
function RabbitMQPool (conf, connection, poolOptions) {
    ...
    pool = genericPool({
        name: 'rabbitmq-' + conf.name + '-' + Math.floor(Math.random() * (9999 - 1001) + 1000),
        ...
        create: function (callback) {
            connection.createChannel()
                .then(function (channel) {
                    ...
                    var _consume = channel.consume;

                    // shim to monitor the name of the consumed queue
                    channel.consume = function (queueName) {
                        return _consume.apply(channel, arguments).then(function (consumer) {
